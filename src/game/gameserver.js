...
module.exports = class GameServer {
  constructor(
    ...
    this.onAccountEnter(function (account) {
      ...
      if (
        Date.now() < account.player.gameserverevent &&
        account.player.server_check === self.id
      ) {
      ...
      if (self.chathistory !== null) {
        ...
        // say hello message, event...
        // if (self.evento200 === true) {
        //   var w = self.SecondsToString(
        //     parseInt(account.player.gameserverevent)
        //   );
        //   data.push([
        //     "¡EVENTO! GP & Gold: 300% - " + w + " para finalizar.",
        //     "",
        //     17,
        //   ]);
        // }
        ...
        // } else if (self.id === 2) {
        //   data.push([
        //     "(gift) ¡ Ya esta todo listo !, Prepárense. (party)",
        //     "Halloween",
        //     5,
        //   ]);
        // } else {
      ...
      account.onExit(function () {
        ...
        if (account.player.room_number !== 0) {
  ...
  guildMessage(account, message) {
    ...
    var chatmessage = [
      Types.SERVER_OPCODE.pchat,
      0,
      account.player.game_id,
      message,
      0,
      account.player.guild,
      0,
    ];
  ...
  sendRooms(account) {
    ...
    self.getRoomsArray({ free: false }, function (arr) {
      data = arr;
  ...
  sendRoomUpdate(room, account) {
    ...
    account.send([
      Types.SERVER_OPCODE.room_update,
      data,
      self.getRoomPosition(room),
    ]);
  }

  sendRoomsType(account, page, options = {}) {
    var self = this;
    var data_fin = [];
    ...
    self.getRoomsArray({ free, status, mode }, function (arr) {
      if (arr !== null) {
  ...
  getRoomsArray({ free, status, mode, }, callback) {
    var self = this;
    var data = [];

    self.forEachRooms(function (rom) {
      if (free === true) {
        if (rom.status === status || rom.game_mode === mode) {
