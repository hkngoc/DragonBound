...
module.exports = class Shoot {
  ...
  GetAngleAtTime(time) {
    const delta = 5;
    var b = this.getPosAtTime(time - delta);
  ...
  positionToForce(
    ...
    let V0Y = Math.round(
      (final - inicial - tiempo * tiempo * weight) /
        (tiempo * Math.sin(ang * Math.PI * 180))
    );
  ...
  setWindEfect() {
    let b0 =
      Math.round(
        parseInt(
          Math.cos((this.wind_ang * Math.PI) / 180) *
            this.wind_power *
            this.weight
        )
      ) / 100;
    let b1 =
      Math.round(
        parseInt(
          Math.sin((this.wind_ang * Math.PI) / -180) *
            this.wind_power *
            this.weight -
            this.friccion
        )
  ...
  autoPower() {
    ...
    for (var i = 0; i < 900; i++) {
      ...
      for (var t = 0; t < 2000; t++) {
        ...
        var cl = Math.sqrt(
          Math.pow(this.x1 - f.x, 2) + Math.pow(this.y1 - f.y, 2)
        );
        if (cl < (60 && this.cl)) {
