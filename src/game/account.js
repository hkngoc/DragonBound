...
module.exports = class Account {
  constructor(connection, gameserver, ip_actions) {
    ...
    connection.onClose(function () {
      ...
      if (self.ignoredisconnect) return false;
  ...
  disconnect(cc = false) {
    if (this.room) {
      ...
      } else this.room.removePlayer(this);
      //console.log("checking func after disconnect");
    }
    var lucky_egg_left = this.lucky_egg_left();
    this.gameserver.db.updateLuckyEggLeft(
      this.user_id,
      lucky_egg_left > 0 ? lucky_egg_left : 0
    );
  ...
  unlocknewbot() {
    ...
    if (new_bot_id < Types.COMPUTER_PLAYER.length) {
      ...
      setTimeout(function () {
        self.send([
          Types.SERVER_OPCODE.alert2,
          59,
          [bot.name, [bot.avatar.h, bot.avatar.b, null, null]],
        ]);
      }, 2000);
    }
  }
  ...
  Handler(opcode, message) {
    ...
    switch (opcode) {
      case Types.CLIENT_OPCODE.login: {
        ...
        if (typeof _id !== "number") {
          self.sendMessage(
            new Message.alertResponse(":)", "Que haces? Ctrl+F5")
          );
        ...
        self.gameserver.db
          .getPlayerData(_id, _session)
          .then(function (data) {
            if (data.banned === 1) {
              self.gameserver.db
                .getUserByBannedTest(data.user_id)
                .then(function (dbplaytt) {
                  var ban_inf = dbplaytt[0][0];
                  self.send([
                    17,
                    "Estás Baneado",
                    "Motivo: " + ban_inf.razon + "<br><br>Hasta: undefined",
                  ]);
              ...
              self.sendMessage(
                new Message.alertResponse(":)", "Error Servidor!")
              );
              ...
              self.gameserver.checkAccountOnlineAndClose(
                data.user_id,
                function () {
                  var otheraccount = self.gameserver.searchAccountById(
                    data.user_id
                  );
                  if (otheraccount !== null) {
                    otheraccount.send([
                      Types.SERVER_OPCODE.disconnect_reason,
                      3,
                      null,
                    ]);
                  ...
                  self.lucky_egg_sec_left =
                    data.lucky_egg_sec_left == ""
                      ? 0
                      : parseInt(data.lucky_egg_sec_left);
                  ...
                  if (self.player.rank >= 0) {
                    ...
                    let cash = 0;
                    let gift_rank = 0;
                    let self2 = self.gameserver.getAccountById(
                      parseInt(self.player.user_id)
                    );
                    let check_ranks = JSON.parse(
                      self.player.first_important_ranks
                    );
                    ...
                    async function getCurrentRank() {
                      try {
                        current_rank =
                          await self.gameserver.db.getPlayerCurrentRank(
                            self.player.user_id
                          );
                        previous_rank =
                          await self.gameserver.db.getPlayerPreviousRank(
                            self.player.user_id
                          );
                        processRanks();
                      } catch (error) {
                    ...
                    function processRanks() {
                      ...
                      self.gameserver.db
                        .getPlayerPreviousRank(self.player.user_id)
                        .then((previous_rank) => {
                          if (
                            self.player.rank > 11 &&
                            ...
                            previous_rank = self.player.rank;
                            self.gameserver.db.updatePreviousRank(
                              previous_rank,
                              self.player.user_id
                            );
                            self.gameserver.db
                              ...
                              .catch(function (err) {
                                ...
                                var name_ava_gift =
                                  self.gameserver.avatars.getAvatagift(
                                    gift_rank
                                  );
                                self.sendMessage(
                                  new Message.alert2Response(
                                    Types.ALERT2_TYPES.RECEIVED_AVATAR,
                                    [
                                      "FunnyBound",
                                      gift_rank,
                                      0,
                                      "Congratulations you have leveled up and thanks to your effort we will reward you with this avatar",
                                      "forever",
                                      name_ava_gift,
                                    ]
                                  )
                                );
                                self.gameserver.db.sendCash(
                                  cash,
                                  self.player.user_id
                                );
                                self2.send([
                                  17,
                                  "Received Cash! :)",
                                  "You just received <font color='yellow'>" +
                                    cash +
                                    "</font> Cash from<br><font color='yellow'>" +
                                    self.player.game_id +
                                    "</font>.<br><br>And cash for Rank up <font color='cyan'><span class='span_rank rank rank" +
                                    next_rank +
                                    "'></span></font>.<br><br>Thank You!",
                                ]);
                      ...
                      if (self.player.rank != next_rank) {
                        if (self.player.gender === "f") tmpgender = 1;
                        if (self.player.rank <= 24) {
                          self.gameserver.db.updateRankByIdAcc(
                            next_rank,
                            self.player.user_id
                          );
                          ...
                          self.gameserver.db
                            ...
                            .catch(function (err) {
                              ...
                              self.gameserver.db.putUserAvatars(datasendgift);
                              var name_ava_gift =
                                self.gameserver.avatars.getAvatagift(gift_rank);
                              self.sendMessage(
                                new Message.alert2Response(
                                  Types.ALERT2_TYPES.RECEIVED_AVATAR,
                                  [
                      ...
                      if (self.player.power_user === 1) {
                        self.gameserver.db
                          .getAvatarExpireExItem(self.player.user_id, 464)
                          .then(function (data) {})
                          .catch(function (err) {
                            self.gameserver.db.updatePowerUser(
                              0,
                              self.player.user_id
                            );
                  ...
                  self.gameserver.db.updateServerByUserId(
                    self.gameserver.id,
                    self.player.user_id
                  );
      ...
      case Types.CLIENT_OPCODE.chat: {
        ...
        //}
        var showm = true;
        if (_msj[0] == /*=*/ "/" /* && self.player.gm === 1*/) showm = false;
      ...
      case Types.CLIENT_OPCODE.send_bcm: {
        ...
        if (
          ...
          self.player.is_muted >= Date.now()
        ) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.MUTED, [])
          );
          return null;
        } else if (self.player.megaphones === 0) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.NEED_ITEM, [
              894,
              1,
              0,
              "Megaphone / Horn / Bugle [ExItem]",
            ])
          );
          return null;
        } else if (self.player.rank < 15) {
          self.sendMessage(
            new Message.alertResponse(
              "Hola " + this.player.game_id,
              "Tu Nivel <span class='span_rank rank rank" +
                self.player.rank +
                "'></span> Es Muy Bajo Para Utilizar El MegaPhone, El Nivel Especial Debe De Ser Mayor <span class='span_rank rank rank15'></span> Para Que Lo Puedas Utilizar"
            )
          );
          return null;
          ...
          _msj = _msj.replace("//", "");
          _msj = _msj.replace("%", "");
          let data = new Message.chatResponse(
            self,
            _msj,
            Types.CHAT_TYPE.BUGLE
          );
          self.gameserver.pushBroadcast(data);
          self.gameserver.chathistory.push([
            "(rank" +
              self.player.rank +
              ") " +
              self.player.game_id +
              ...
              self.gameserver.db.removeMegaphone(self.user_id, function () {
                ...
                self.gameserver.db.getPlayerAvatars(self).then(function (data) {
                  if (data.error_mysql || data.error_querry) {
                    ...
                    var dat = self.gameserver.avatars.getAvatarDataList(
                      data.data_list
                    );
                    self.sendMessage(new Message.myAvatars(self, dat));
                  }
      ...
      case Types.CLIENT_OPCODE.pchat: {
        ...
        if (typeof _id !== "number") return null;
        let _msj = message[2];
        if (_id == 0) {
          ...
          if (typeof account !== "undefined" && account !== null) {
            account.sendMessage(
              new Message.pChatResponse(self, this.player.game_id, _msj)
            );
            self.sendMessage(
              new Message.pChatResponse(account, this.player.game_id, _msj)
            );
            Logger.info(
              "Chat Privado [De: " +
                self.player.game_id +
            ...
            if (accountback != undefined) {
              ...
              self.sendMessage(
                new Message.pChatResponse(
                  accountback,
                  "Offline",
                  " PM será entregado cuando el usuario inicie sesión."
                )
              );
              if (self.gameserver.pending_messages[_id] == undefined)
                self.gameserver.pending_messages[_id] = [];
              self.gameserver.pending_messages[_id].push(
                new Message.pChatResponse(
                  self,
                  this.player.game_id,
                  _msj
      ...
      case Types.CLIENT_OPCODE.get_shop_page: {
        ...
        if (message[1].length > 1) {
          data_page = self.gameserver.avatars.getShopListType(
            type,
            page,
            true,
            message[1],
      ...
      case Types.CLIENT_OPCODE.buy: {
            ...
            if (account !== null) {
              if (account.player.game_id === user_gift) {
                if (account.player.gender === "f") {
                  ...
                  self.player.gender === "f" &&
                  account.player.gender === "m"
                ) {
                  tmpgender = 0;
                } else {
                }
        ...
        if (item_data) {
          ...
          var rankspecial = 0;
          var megaponeuses = 0;
          if (item_data[6] === "") errtrampa = true;
          if (item_data[6].min_rank > self.player.rank) {
            self.sendMessage(
              new Message.alert2Response(Types.ALERT2_TYPES.NOT_FOR_SELL, [])
            );
            return null;
          }
          if (period === Types.PERIOD.WEEK) {
            ...
            if (is_cash) {
              ...
              if (item_data[6].cash_week <= 0) errtrampa = true;
            } else {
              _iprecio = item_data[6].gold_week;
              if (item_data[6].gold_week <= 0) errtrampa = true;
            }
            ...
            if (is_cash) {
              ...
              if (item_data[6].cash_month <= 0) errtrampa = true;
            } else {
              _iprecio = item_data[6].gold_month;
              if (item_data[6].gold_month <= 0) errtrampa = true;
            }
            ...
            if (is_cash) {
              ...
              if (item_data[6].cash_perm <= 0) errtrampa = true;
            } else {
              _iprecio = item_data[6].gold_perm;
              if (item_data[6].gold_perm <= 0) errtrampa = true;
            }
          }
          if (is_cash) {
            if (self.player.cash >= _iprecio) valid_precio = true;
          } else {
            if (self.player.gold >= _iprecio) valid_precio = true;
          }
          if (
            (id === 1066 && self.player.gm === 1) ||
            (id === 1067 && self.player.gm === 1) ||
            (id === 1068 && self.player.gm === 1)
          )
            return null;
          if (user_gift !== "") {
            self.gameserver.db.getUserByGameId(user_gift).then(function (rows) {
              ...
              let self2 = self.gameserver.getAccountById(
                parseInt(info_user.IdAcc)
              );
              if (
                (id === 1066 && self2.player.gm === 1) ||
                (id === 1067 && self2.player.gm === 1) ||
                (id === 1068 && self2.player.gm === 1)
              )
                return null;
              ...
              if (valid_precio && !errtrampa) {
                ...
                if (self.player.rank < 17) {
                  self.sendMessage(
                    new Message.alertResponse(
                      "Lo sentimos :(",
                      "Tu nivel no es suficiente para enviar el regalo.",
                      []
                    )
                  );
                } else {
                  self.sendMessage(
                    new Message.alert2Response(Types.ALERT2_TYPES.GIFT_SENT, [
                      id,
                    ])
                  );
                  Promise.all([self.gameserver.db.putUserAvatars(data)]).then(
                    (data) => {
                      self2.gameserver.db
                        .getPlayerAvatars(self2)
                        .then(function (data) {
                          if (data.error_mysql || data.error_querry) {
                          } else {
                            var dat =
                              self2.gameserver.avatars.getAvatarDataList(
                                data.data_list
                              );
                            self2.sendMessage(
                              new Message.myAvatars(self2, dat)
                            );
                          }
                        });
                    }
                  ...
                  try {
                    var name_ava_gift =
                      self.gameserver.avatars.getAvatagift(id);
                    self2.sendMessage(
                      new Message.alert2Response(
                        Types.ALERT2_TYPES.RECEIVED_AVATAR,
                        [
                    ...
                    if (id === 894) {
                      self.gameserver.db.updateMegaPone(
                        megaponeuses,
                        self2.player.user_id
                      );
                    ...
                    if (aceptrankspecial === "se") {
                      self.gameserver.db.updateRankSpecialByIdAcc(
                        rankspecial,
                        1,
                        self2.player.user_id
                      );
                      self.gameserver.db.putSpecialRanksByUserId(
                        self2.player.user_id,
                        self2.player.game_id,
                        rankspecial,
                        _iprecio,
                        timerank
                      );
                  ...
                  if (is_cash === true) {
                    ...
                    self.gameserver.db.sendDeleteCash(
                      0,
                      _iprecio,
                      self.player.user_id
                    );
            ...
            if (valid_precio && !errtrampa) {
              ...
              if (_iprecio <= 0) {
                ...
                self.gameserver.db
                  .getBoy(self.player.user_id, id)
                  .then(function (acc) {
                    self.sendMessage(
                      new Message.alert2Response(
                        Types.ALERT2_TYPES.ALREADY_HAVE,
                        []
                      )
                    );
                  })
                    ...
                    self.gameserver.db
                      ...
                      .then(function (data) {
                        if (data.error_mysql || data.error_querry) {
                          ...
                          self.sendMessage(
                            new Message.alert2Response(
                              Types.ALERT2_TYPES.PURCHASED,
                              [id]
                            )
                          );
                          self.gameserver.db
                            .getPlayerAvatars(self)
                            .then(function (data) {
                              if (data.error_mysql || data.error_querry) {
                              } else {
                                var dat =
                                  self.gameserver.avatars.getAvatarDataList(
                                    data.data_list
                                  );
                                self.sendMessage(
                                  new Message.myAvatars(self, dat)
                                );
                              }
                            });
                          if (id === 464) {
                          ...
                          if (id === 893) {
                            self.gameserver.db.updatePlusGP(
                              self.player.user_id
                            );
                          ...
                          if (is_cash === true)
                            self.player.cash = parseInt(
                              self.player.cash - _iprecio
                            );
                          else
                            self.player.gold = parseInt(
                              self.player.gold - _iprecio
                            );
                          self.sendMessage(new Message.loginResponse(self));
                        }
                      })
          ...
          self.sendMessage(
            new Message.alert2Response(
              Types.ALERT2_TYPES.AVATAR_WRONG_GENDER,
              []
            )
          );
        }
      ...
      case Types.CLIENT_OPCODE.delete_avatar: {
        ...
        self.gameserver.db
          ...
          .then(function (data) {
            ...
            if (item == 894) {
              self.gameserver.db
                .DeleteAvatarX(self.player.user_id, 5)
                .then(() => {});
              self.player.megaphones = 0;
            } else if (item == 464) {
              self.gameserver.db
                .DeleteAvatarX(self.player.user_id, 1)
                .then(() => {});
              self.player.power_user = 0;
              self.sendMessage(new Message.loginResponse(self));
            } else if (item == 893) {
              self.gameserver.db
                .DeleteAvatarX(self.player.user_id, 2)
                .then(() => {});
              self.player.plus10gp = 0;
              self.sendMessage(new Message.loginResponse(self));
            } else if (item == 1223) {
              self.gameserver.db
                .DeleteAvatarX(self.player.user_id, 3)
                .then(() => {});
        ...
        if (self.player.room_number === 0) {
          ...
          self.gameserver.forEachRooms(function (roomss) {
            ...
            if (roomss.status === Types.ROOM_STATUS.WAITING) {
              if (
                roomss.player_count < roomss.max_players &&
                roomss.status === Types.ROOM_STATUS.WAITING
              ) {
          ...
          var room_random =
            data_room[Math.floor(Math.random() * data_room.length)];
          ...
          self.gameserver.getRoomById(parseInt(room_random), function (room) {
            if (room) {
              if (
                ...
                !room.team_a[self.user_id] &&
                !room.team_b[self.user_id]
              ) {
                if (room.password === "") {
      ...
      case Types.CLIENT_OPCODE.equip: {
        ...
        if (work) {
          self.gameserver.db.equipAvatar(arr_up, self).then(function (data) {
            if (data.error_mysql || data.error_querry) {
              ...
              if (self.room) {
                self.gameserver.pushToRoom(
                  self.room.id,
                  new Message.roomPlayers(self.room),
                  null
                );
              }
              self.sendMessage(new Message.loginResponse(self));
          ...
          self.sendMessage(new Message.loginResponse(self));
          self.gameserver.sendAccountsOnline();
          self.gameserver.db.defaultAvatars(
            self.player.reg_id,
            self.player.ahead,
            self.player.abody
          );
      ...
      case Types.CLIENT_OPCODE.event: {
        ...
        self.gameserver.db
          .getEventLogByIdAcc(self.player.user_id)
          .then(function (e) {
            let ex = e[0][0];
            ...
            if (type === 0) {
              if (ex.Event1 - Date.now() / 1000 > 0) {
                self.player.event1 =
                  ex.Event1 - Date.now() / 1000 < 0
                    ? 0
                    : ex.Event1 - Date.now() / 1000;
                self.sendMessage(new Message.loginResponse(self));
                return null;
              } else {
                ...
                self.player.event1 =
                  ti - Date.now() / 1000 < 0 ? 0 : ti - Date.now() / 1000;
                self.gameserver.db
                  .eventS(1, ti, self.player.user_id)
                  .then(function (e) {})
                  .catch(function (err) {
                    //console.log("err")
                  });
                self.gameserver.db.updateGoldCashEventByIdAcc(
                  giftEvent.event1.gold,
                  giftEvent.event1.cash,
                  self.player.user_id
                );
                self.sendMessage(
                  new Message.alert2Response(Types.ALERT2_TYPES.WON_EVENT1, [
                    giftEvent.event1.cash,
                    giftEvent.event1.gold,
                    data,
                  ])
              ...
              if (ex.Event2 - Date.now() / 1000 > 0) {
                self.player.event2 =
                  ex.Event2 - Date.now() / 1000 < 0
                    ? 0
                    : ex.Event2 - Date.now() / 1000;
                self.sendMessage(new Message.loginResponse(self));
                return null;
              } else {
                ...
                self.player.event2 =
                  ti2 - Date.now() / 1000 < 0 ? 0 : ti2 - Date.now() / 1000;
                self.gameserver.db
                  .eventS(2, ti2, self.player.user_id)
                  .then(function (e) {})
                  .catch(function (err) {
                    //console.log("err")
                  });
                self.gameserver.db.updateGoldCashEventByIdAcc(
                  giftEvent.event2.gold,
                  giftEvent.event2.cash,
                  self.player.user_id
                );
                self.sendMessage(
                  new Message.alert2Response(Types.ALERT2_TYPES.WON_EVENT2, [
                    giftEvent.event2.cash,
                    giftEvent.event2.gold,
                    data,
                  ])
      ...
      case Types.CLIENT_OPCODE.change_name: {
        ...
        if (ignoreCase.startsWith(_nname, " ")) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.NAME_BAD_CHAR, [])
          );
          return null;
        }
        ...
        if (
          ignoreCase.startsWith(_nname, "GM") ||
          ignoreCase.startsWith(_nname, "BattleFunny")
        ) {
          fuck = true;
        }
          ...
          for (let i = 0; i < Types.GAME_ID.length; i++) {
            if (ignoreCase.equals(_nname, Types.GAME_ID[i])) {
              ...
              self.sendMessage(
                new Message.alertResponse(
                  "Prohibited",
                  "you are prohibited from changing your game nickname"
                )
              );
              return null;
            }
        ...
        if (self.player.gm === 1) fuck = false;
        if (fuck) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.NAME_BAD_CHAR, [])
          );
          ...
          self.gameserver.db
            ...
            .catch(function (data) {
              if (data.error_mysql || data.error_querry) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.NAME_BAD_CHAR,
                    []
                  )
                );
                ...
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.NAME_ALREADY_EXISTS,
                    []
                  )
                );
                ...
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.NAME_NOT_ENOUGH_CASH,
                    []
                  )
                );
              }
          ...
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.NAME_BAD_LEN, [])
          );
        }
        break;
      }
      ...
      case Types.CLIENT_OPCODE.tab: {
          ...
          var check_ready = function () {
            if (trys < 400)
              self.login_complete
                ...
                : (setTimeout(check_ready, 10), trys++);
          };
          check_ready();
          return null;
        }
        ...
        if (slot === 0) {
          ...
          self.gameserver.db
            .getFriendsByIdyo(self.player.user_id)
            .then(function (rows) {
              ...
              for (var ix in my_friends) {
                ...
                if (my_friends[ix].IsOnline !== 0) {
                  if (self.gameserver.id === my_friends[ix].IsOnline) {
                    var my_friend_room = self.gameserver.getAccountById(
                      parseInt(my_friends[ix].IdAcc)
                    );
                ...
                var fserver = 0;
                let account = self.gameserver.searchAccountById(
                  my_friends[ix].IdAcc
                );
                if (account !== null) {
              ...
              //self.sendMessage(new Message.loginResponse(self));
              self.send([
                Types.SERVER_OPCODE.friends,
                my_friends_x2,
                self.gameserver.id,
                parseInt(my_friends.length + 1),
              ]);
            })
            .catch(function () {
              self.send([
          ...
          if (self.player.guild !== "") {
            self.gameserver.db
              .getGuildMembersById(self.player.guild_id)
              .then(function (rows) {
      ...
      case Types.CLIENT_OPCODE.guild_create: {
        ...
        let gname = message[1];
        var info_prix = ArrayToObject(
          self.player.tournament,
          "start_time end_time players avatar_on max_wind force_mobile name total_games last_5_minutes_games rooms maps game_mode s1 tp save_personal save_guild min_points different_mobiles gifts gp_event turn_time".split(
            " "
          )
        );
        if (self.player.guild !== "") {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.ALREADY_IN_GUILD, [])
          );
        } else if (
          info_prix.players === 7 &&
          this.tournament_start_time_server <= Date.now() &&
          this.tournament_end_time_server >= Date.now()
        ) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.GUILDS_LOCK, [])
          );
          return null;
          ...
          if (/\s/g.test(gname) === true) {
            self.sendMessage(
              new Message.alert2Response(
                Types.ALERT2_TYPES.GUILD_NAME_BAD_WORD,
                []
              )
            );
            return null;
          ...
          if (self.player.gold >= 500000 === false) {
            self.sendMessage(
              new Message.alert2Response(Types.ALERT2_TYPES.GUILD_NO_MONEY, [])
            );
            return null;
          }
          self.gameserver.db
            .createGuild(gname, self)
            .then(function (data) {
              self.sendMessage(
                new Message.alert2Response(Types.ALERT2_TYPES.GUILD_CREATED, [])
              );
              self.sendMessage(new Message.loginResponse(self));
              /* =================================================================== */
              self.gameserver.db
            ...
            .catch(function (data) {
              if (data.error_mysql || data.error_querry) {
                ...
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.GUILD_ALREADY_EXISTS,
                    []
                  )
                );
              }
          ...
          self.sendMessage(
            new Message.alert2Response(
              Types.ALERT2_TYPES.GUILD_BAD_NAME_LEN,
              []
            )
          );
        }
      ...
      case Types.CLIENT_OPCODE.guildinvite: {
        ...
        if (
          ...
          self.sendMessage(
            new Message.alertResponse(
              "Lo sentimos",
              "Esta opción del Guild esta bloqueada durante el torneo. Inténtalo mas tarde."
            )
          );
          return null;
        } else {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.GUILD_INVITE_SENT, [
              acc.player.game_id,
            ])
          );
          if (acc) {
          ...
          self.gameserver.db
            ...
            .then(function (data) {
              if (data.good) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.JOINED_GUILD,
                    []
                  )
                );
                if (self.room) {
      ...
      case Types.CLIENT_OPCODE.guild_leave: {
        ...
        if (
          (self.player.guild !== "" && self.player.guild_job === 0) ||
          (self.player.guild !== "" && self.player.guild_job === 2)
        ) {
          self.gameserver.db
            .leaveGuild(self.player.user_id)
            .then(function (data) {
              if (data.complete) {
                self.player.guild = "";
        ...
        if (self.player.guild_job === 1) {
          self.gameserver.db
            .DeleteNameGuild(self.player.guild, self.player.guild_id)
            .then(function (data) {
              if (data.complete) {
                self.player.guild = "";
                self.player.guild_job = 0;
                ...
                if (self.room) {
                  ...
                  self.sendMessage(
                    new Message.alert2Response(
                      Types.ALERT2_TYPES.CLOSED_GUILD,
                      []
                    )
                  );
                }
      ...
      case Types.CLIENT_OPCODE.guild_kick: {
        ...
        if (self.user_id === guser_id) {
          self.sendMessage(
            new Message.alert2Response(
              Types.ALERT2_TYPES.CANT_KICK_YOURSELF,
              []
            )
          );
        } else if (self.player.guild !== "" && self.player.guild_job === 1) {
          self.gameserver.db
            .kickGuild(guser_id, self.player.guild_id)
            .then(function (data) {
              if (data.complete) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.KICKED_GUILD,
                    []
                  )
                );
              }
      ...
      case Types.CLIENT_OPCODE.channel_rooms: {
        ...
        if (_strtype === "all") {
          ...
          self.player.channel_rango = self.player.channel_rango - 6;
          if (self.player.channel_rango < 0) self.player.channel_rango = 0;
      ...
      case Types.CLIENT_OPCODE.getinfo: {
        ...
        if (acc) {
          self.gameserver.db
            .getMyFriend(self.player.user_id, acc.player.user_id)
            .then(function (rows) {
        ...
        if (self.room) {
          ...
          if (
            ...
            if (self.gameserver.name === "Holiday") {
              ...
              self.sendMessage(new Message.loginResponse(self));
              self.gameserver.db
                .updatePrix(self.player.punts_prix_user, self)
                .then(function (data) {
                  if (data.error_mysql || data.error_querry) {
                  } else {
            ...
            self.gameserver.db.updateLeftByIdAcc(
              1500,
              5,
              1,
              self.player.user_id
      ...
      case Types.CLIENT_OPCODE.room_watch: {
        ...
        self.gameserver.getRoomById(room_id, function (room) {
          if (room) {
            ...
            self.sendMessage(
              new Message.alert2Response(
                Types.ALERT2_TYPES.ROOM_DOES_NOT_EXIST,
                []
              )
            );
          }
      ...
      case Types.CLIENT_OPCODE.room_join: {
        ...
        if (self.room != undefined) return false;
        var info_prix = ArrayToObject(
          self.player.tournament,
          "start_time end_time players avatar_on max_wind force_mobile name total_games last_5_minutes_games rooms maps game_mode s1 tp save_personal save_guild min_points different_mobiles gifts gp_event turn_time".split(
            " "
        ...
        self.gameserver.getRoomById(id, function (room) {
          if (room) {
            if (room.search_team_room === 1 && room.player_count >= 4) {
              self.sendMessage(
                new Message.alert2Response(Types.ALERT2_TYPES.ROOM_FULL, [])
              );
              return null;
            } else if (
              room.search_team_room === 1 &&
              room.room_tournament_playing === 1
            ) {
              self.sendMessage(
                new Message.alert2Response(Types.ALERT2_TYPES.ROOM_PLAYING, [])
              );
              return null;
            } else if (
              typeof room.kick_user_time[parseInt(self.player.user_id)] !==
              "undefined"
            ) {
              if (
                parseInt(room.kick_user_time[self.player.user_id].expiry) >=
                Date.now()
              ) {
                self.sendMessage(
                  new Message.alert2Response(Types.ALERT2_TYPES.KICKED, [
                    parseInt(
                      secondsremaining(
                        room.kick_user_time[parseInt(self.player.user_id)]
                          .expiry
                      )
                    ),
                  ])
                ...
                var delete_kick = setTimeout(function () {
                  ...
                  Math.round(
                    secondsremaining(
                      room.kick_user_time[parseInt(self.player.user_id)].expiry
                    ) * 1000
                  )
                ));
                return null;
              ...
              if (
                ...
                if (room.password !== password)
                  self.sendMessage(
                    new Message.alert2Response(
                      Types.ALERT2_TYPES.WRONG_PASSWORD,
                      []
                    )
                  );
                else {
                ...
                if (self.gameserver.name === "Guilds Prix") {
                  if (
                    info_prix.force_mobile !== -1 ||
                    info_prix.force_mobile !== -2
                  ) {
      ...
      case Types.CLIENT_OPCODE.room_create: {
        ...
        self.gameserver.rooms[id] = new Room(
          id,
          title,
          password,
          maxplayers,
        ...
        self.gameserver.getRoomById(id, function (room) {
          if (room) {
            if (room.player_count < room.max_players) {
              ...
              if (self.player.power_user === 1) room.power = 1;
            }
          } else {
            self.sendMessage(
              new Message.alert2Response(
          ...
          if ((self.player.is_master = 1)) {
            ...
            if (self.room.game_mode === Types.GAME_MODE.BOSS) {
              if (self.room.player_count <= 4) {
                ...
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.CANT_BOSS_PLAYERS,
                    []
                  )
                );
              }
      ...
      case Types.CLIENT_OPCODE.tournament_start_game: {
        ...
        if (
          typeof Types.MOBILES[mobile_prix] != "undefined" &&
        ...
        if (self.gameserver.name === "Guilds Prix") {
          self.sendMessage(
            new Message.alertResponse("I am sorry", "4v4 Guild vs Guild Games")
          );
          return null;
        }
        ...
        if (self.gameserver.name === "Prix") {
          if (self.player.tournament_start_time_server >= Date.now()) {
            self.sendMessage(
              new Message.alert2Response(
                Types.ALERT2_TYPES.TOURNAMENT_NOT_STARTED,
                []
              )
            );
            return null;
          ...
          if (
            self.player.tournament_start_time_server <= Date.now() &&
            self.player.tournament_end_time_server <= Date.now()
          ) {
            self.sendMessage(
              new Message.alert2Response(
                Types.ALERT2_TYPES.TOURNAMENT_ENDED,
                []
              )
            );
            return null;
          ...
          if (self.player.punts_prix_user <= info_prix.min_points) {
            self.sendMessage(
              new Message.alert2Response(
                Types.ALERT2_TYPES.DISQUALIFIED_PLAYER,
                [self.player.punts_prix_user, info_prix.min_points]
              )
            );
            return null;
          }
        ...
        self.send([
          Types.SERVER_OPCODE.tournament_wait,
          parseInt(getRndInteger(1, 3)),
        ]); //self.send([41,parseInt(getRndInteger(1, 3))]);
        //let id = self.gameserver.getIdforRoom();
        ...
        if (self.player.tournament_wait_game == 1) {
          ...
          setTimeout(function () {
            if (
              (self.location_type === Types.LOCATION.CHANNEL &&
                self.player.tournament_wait_game == 1 &&
                self.gameserver.id === 2) ||
              (self.location_type === Types.LOCATION.CHANNEL &&
                self.player.tournament_wait_game == 1 &&
                self.gameserver.name === "Bunge.")
            ) {
              let id = self.gameserver.getIdforRoom();
              self.gameserver.rooms[id] = new Room(
                id,
                "(time) BOT Computer (time)",
                "",
                1,
                1,
                self.gameserver
          ...
          let random_probability_holiday = parseInt(
            getRndInteger(3, endpoints)
          );
          ...
          if (
            self.gameserver.name === "Holiday" &&
            self.player.tournament_wait_game == 1 &&
            self.player.gm_probability ===
              random_probability_holiday /* || self.player.user_id === 1*/
          ) {
            let id = self.gameserver.getIdforRoom();
            self.gameserver.rooms[id] = new Room(
              id,
              "(party) Halloween 2021 (party)",
              "",
              1,
              1,
              self.gameserver
            ...
            self.gameserver.getRoomById(id, function (room) {
              if (room) {
                ...
                room.game.onGameEnd(function (teamm) {
                    ...
                    if (typeof accountdbb !== "undefined") {
                      ...
                      if (teamm === playerdbb.team) {
                        ...
                        if (accountdbb.scores_lose !== 1) {
                          if (accountdbb.user_id >= 80000) return null;
                          ...
                          self.gameserver.db
                            .getUserAvatarsByIdAccANDaId(
                              playerdbb.user_id,
                              Ava_Send
                            )
                            .then(function (rowss) {})
                            .catch(function (err) {
                              var name_ava_gift_hd =
                                self.gameserver.avatars.getAvatagift(
                                  Santa_Claus_Head
                                );
                              var name_ava_gift_bd =
                                self.gameserver.avatars.getAvatagift(
                                  Santa_Claus_Body
                                );
                              let self2 = self.gameserver.getAccountById(
                                parseInt(playerdbb.user_id)
                              );
                              ...
                              accountdbb.send([
                                ...
                                "You just received <font color='yellow'>" +
                                  ...
                                  "</font> Cash from<br><font color='yellow'>" +
                                  accountdbb.game_id +
                                  "</font>.<br><br>Thank You!",
                              ]);

                              accountdbb.gameserver.pushBroadcast(
                                new Message.chatResponse(
                                  self,
                                  plx.game_id +
                                    " sent gift -> " +
                                    name_ava_gift_hd +
                          ...
                          self.gameserver.db.updateProbability(
                            playerdbb.gm_probability,
                            playerdbb.user_id
                          );
                        }
                      } else {
                  ...
                  room.game = null;
                  room.status = Types.ROOM_STATUS.WAITING;
                  room.gameserver.pushToRoom(
                    room.id,
                    new Message.roomPlayers(room)
                  );
                  setTimeout(function () {
                    self.player.room_number = 0;
                    self.location_type = Types.LOCATION.CHANNEL;
                    ...
                    self.gameserver.forEachAccount(function (accounttp) {
                      if (
                        accounttp !== null &&
                        accounttp.player.user_id === self.player.user_id
                      ) {
                        var data_game_server =
                          self.gameserver.chathistory.slice(0);
                        data_game_server.push(["", "", 9]);
                        if (self.gameserver.evento200 === true)
                          data_game_server.push([
                            " El porcentaje de GP & Gold cambiaron a 200%",
                            "[Inicio de Evento",
                            17,
                          ]);
                        if (self.gameserver.id === 2) {
                          data_game_server.push([
                            " Búscame, gáname y te llevas un regalo: (gift) " +
                              accounttp.player.gifts_holiday +
                              " Regalos enviados (gift)",
                            "Halloween",
                            5,
                          ]);
                          data_game_server.push([
                            " Tienes " +
                              accounttp.player.gm_probability +
                              " ganadas seguidas = 200% GP & Gold! Event probabilidad x" +
                              accounttp.player.gm_probability,
                            "",
                            6,
                          ]);
          ...
          self.gameserver.forEachAccount(function (accountp) {
            ...
            if (accountp !== null ? accountp.user_id != self.user_id : !1) {
              if (
                accountp.player.tournament_wait_game == 1 &&
                self.player.tournament_wait_game == 1
              ) {
      ...
      case Types.CLIENT_OPCODE.create_team: {
        ...
        if (info_prix.players === 2) {
          self.sendMessage(
            new Message.alertResponse(
              "No teams on this server",
              "Only 1v1 games are allowed on this server right now."
            )
          );
          return null;
        }
          ...
          if (room) {
            if (room.player_count < room.max_players) {
              ...
              if (self.gameserver.name === "Bunge.") {
                ...
                var room_random =
                  data_maps_server[
                    Math.floor(Math.random() * data_maps_server.length)
                  ];
                if (room_random === 40) room_random = 39;
      ...
      case Types.CLIENT_OPCODE.game_share: {
        ...
        if (message[1] === 2) {
          self.gameserver.db
            .getMyScreeRoomGameByLetters(self.player.code_screenshot_random)
            .then(function (rows) {
              self.send([
                Types.SERVER_OPCODE.game_share,
                parseInt(message[1]),
                self.player.code_screenshot_random,
                JSON.stringify(self.player.screenshot),
              ]);
            })
            .catch(function () {
              self.gameserver.db
        ...
        if (message[1] === 1) {
          self.gameserver.db
            .getMyReplayRoomGameByLetters(self.player.code_screenshot_random)
            .then(function (rows) {
              self.send([
                Types.SERVER_OPCODE.game_share,
                parseInt(message[1]),
            ...
            .catch(function () {
              ...
              if (self.room) {
                ...
                self.gameserver.db
                  .putMyReplayRoomGame(
                    self.player.code_screenshot_random,
                    [JSON.stringify(avatars_ids)],
                    JSON.stringify(date_game)
                  )
                  .then(function (rows) {
                    fs.writeFile(
                      "./replays/" +
                        self.player.code_screenshot_random +
                        ".json",
                      JSON.stringify(self.player.view_replay),
                      function (err) {
                        if (err) throw err;
                        self.send([
                          Types.SERVER_OPCODE.game_share,
                  ...
                  .catch(function (err) {
                    Logger.normal(
                      "Game Share [error]: " + err.stack + " - error: " + err
                    );
                  });
              }
            });
      ...
      case Types.CLIENT_OPCODE.room_change_team: {
        ...
        if (self.room) {
          if (self.room.search_team_room === 0) self.room.changeTeam(self);
        }
        break;
      }
      case Types.CLIENT_OPCODE.select_bot: {
        ...
        if (self.room && self.player.is_master) {
          if (self.room.team_bots_count <= 4) {
            if (deleteBot) {
              const newBots = self.room.accountsOfBot.filter(
                (data) => data.position != positionOfBot
              );
              ...
              const account_exist = self.room.accountsOfBot.filter(
                (data) => data.id == bot_id
              );
              ...
              if (bot_exist) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.ALREADY_IN_ROOM,
                    []
                  )
                );
              } else if (
                positionOfBot == 1 &&
                self.room.accountsOfBot.length == 1
              ) {
                self.room.resetBot();
                self.room.accountsOfBot = [];
                ...
                } else self.onCreateBot(changeDataBot);
              } else {
                let newDataBot = { positionOfBot, bot_id };
                if (bot_id > self.player.unlock) {
                  self.send([Types.SERVER_OPCODE.alert2, 12, [10, 26]]);
                } else self.onCreateBot(newDataBot);
              }
            }
          }
        }
      ...
      case Types.CLIENT_OPCODE.mobile: {
        ...
        if (self.room) {
          if (
            typeof Types.MOBILES[_mob] != "undefined" &&
            Types.MOBILES[_mob] !== null
          ) {
            if (
              self.player.is_master === 1 &&
              self.room.game_mode === Types.GAME_MODE.SAME
            ) {
              self.room.forPlayers(function (accountdbp) {
                if (accountdbp !== null) {
                  ...
                  if (
                    ...
                    if (
                      _mob == Types.MOBILE.COPYLOID ||
                      _mob == Types.MOBILE.BEE
                    )
                      prohivido = false;
                  if (prohivido) {
                    if (_mob == Types.MOBILE.RANDOM) {
                      ...
                      self.send([
                        17,
                        "¡Locked!",
                        "No tienes permitido realizar esta función.",
                      ]);
                    } else {
                    ...
                    self.gameserver.pushToRoom(
                      self.player.room_number,
                      new Message.changedMobile(accountdbp)
                    );
                    return null;
                  } else {
          ...
          if (
            ...
            if (_mob == Types.MOBILE.COPYLOID) {
              self.sendMessage(
                new Message.alertResponse(
                  "Non-Selectable Mobile",
                  "You can not select this mobile."
                )
              );
              self.player.mobile = Types.MOBILE.ARMOR;
              self.gameserver.pushToRoom(
        ...
        if (self.room) {
          if (self.room.game_mode === Types.GAME_MODE.BOSS) {
            if (self.room.team_bots_count === 0) {
              self.sendMessage(
                new Message.alertResponse(
                  "Hola " + this.player.game_id,
                  "Para Empezar La Partida Amenos Debe De Eligir Mínimo Un Boss."
                )
              );
              return null;
            }
          ...
          if (self.player.is_master === 1) {
            ...
            if (self.room.search_team_room === 1) {
              ...
              if (
                self.room.player_count < 4 &&
                self.gameserver.name === "Guilds Prix"
              ) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.NOT_4_SAME_GUILD,
                    []
                  )
                );
                return null;
              ...
              if (
                self.room.player_count === 1 &&
                self.gameserver.name !== "Prix"
              ) {
                self.sendMessage(
                  new Message.alert2Response(Types.ALERT2_TYPES.FEW_PLAYERS, [])
                );
                return null;
              } else if (
                self.player.tournament_start_time_server >= Date.now() &&
                ...
                self.sendMessage(
                  new Message.alert2Response(Types.ALERT2_TYPES.NO_GUILD, [])
                );
                return null;
              } else if (
                self.player.guild_score <= info_prix.min_points &&
                self.gameserver.name === "Guilds Prix"
              ) {
                self.sendMessage(
                  new Message.alert2Response(
                    Types.ALERT2_TYPES.DISQUALIFIED_GUILD,
                    [self.player.guild_score, info_prix.min_points]
                  )
                );
                return null;
                  ...
                  if (self.gameserver.name === "Guilds Prix") {
                    if (self.player.guild !== "") {
                      if (account1.player.guild === self.player.guild) {
                        ...
                        self.sendMessage(
                          new Message.alert2Response(
                            Types.ALERT2_TYPES.NOT_IN_MY_GUILD,
                            []
                          )
                        );
                ...
                self.gameserver.forEachRooms(function (rooms_search) {
                  if (self.room.team_tournament_game === 1) {
                    if (self.room.id != rooms_search.id) {
                      if (
                        self.room.team_tournament_game === 1 &&
                        rooms_search.team_tournament_game === 1
                      ) {
                        if (
                          self.room.player_count === rooms_search.player_count
                        ) {
                          if (
                            self.room.room_players_guild !==
                            rooms_search.room_players_guild
                          ) {
                            //
                            self.room.team_tournament_game = 0;
                            rooms_search.team_tournament_game = 0;
                            self.room.forPlayers(function (account1) {
                              account1.player.team_tournament_room =
                                self.room.id;
                            ...
                            rooms_search.forPlayers(function (account2) {
                              account2.player.team_tournament_room =
                                rooms_search.id;
                              ...
                              if (room) {
                                ...
                                if (self.gameserver.name === "Bunge.") {
                                  ...
                                  var room_random =
                                    data_maps_server[
                                      Math.floor(
                                        Math.random() * data_maps_server.length
                                      )
                                    ];
                                  if (room_random === 40) room_random = 39;
                                  room.map = Types.MAPS_PLAY[room_random];
                                } else if (
                                  self.gameserver.name === "Battle Off"
                                ) {
                                  var data_maps_server = info_prix.maps;
                                  var room_random =
                                    data_maps_server[
                                ...
                                self.room.forPlayers(function (account1) {
                                  ...
                                  account1.send([
                                    Types.SERVER_OPCODE.enter_room,
                                  ]);
                                  room.updatePosition().then(function () {
                                    account1.room = room;
                                    account1.sendMessage(
                                      new Message.roomState(room)
                                    );
                                    self.gameserver.pushToRoom(
                                      room.id,
                                      new Message.roomPlayers(room),
              ...
              self.room.forPlayers(function (account) {
                if (
                  self.player.gm != 1 &&
                  self.player.computer_ip === account.player.computer_ip &&
                  self.player.team !==
                    self.gameserver.getAccountById(account.player.user_id)
                      .player.team
                ) {
                  start_game = false;
                }
      ...
      case Types.CLIENT_OPCODE.game_use_item: {
        ...
        if (self.room) {
          ...
          self.gameserver.pushToRoom(
            self.player.room_number,
            new Message.usedItems(self, x__)
          );
          self.send(item_used);
        }
        break;
      ...
      case Types.CLIENT_OPCODE.addfriend: {
        ...
        if (
          (self.player.rank < 26 && amigo_id.player.rank === 26) ||
          (self.player.rank < 26 && amigo_id.player.rank === 27) ||
          (self.player.rank < 26 && amigo_id.player.rank === 31)
        ) {
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.CANT_FRIEND_GM, [])
          );
          return null;
        }
        let ropa = [
        ...
        amigo_id.send([
          Types.SERVER_OPCODE.friendreq,
          [self.player.user_id, self.player.game_id, ropa],
        ]);
        self.sendMessage(
          new Message.alert2Response(Types.ALERT2_TYPES.FRIEND_REQUEST_SENT, [
            amigo_id.player.game_id,
          ])
        );
        /*} else {
      ...
      case Types.CLIENT_OPCODE.friend_approved: {
        ...
        let amigo_ropa = [
          envitando_friend.player.ahead,
          envitando_friend.player.abody,
          envitando_friend.player.aeyes,
          envitando_friend.player.aflag,
          envitando_friend.player.aforeground,
          envitando_friend.player.abackground,
        ];
        self.gameserver.db.putFriends(
          self.player.user_id,
          envitando_friend.player.user_id
        );
        self.sendMessage(
          new Message.alert2Response(Types.ALERT2_TYPES.FRIEND_ADDED, [
            envitando_friend.player.game_id,
            amigo_ropa,
          ])
        );
        self.gameserver.db.putFriends(
      ...
      case Types.CLIENT_OPCODE.friend_delete: {
        ...
        self.gameserver.db.deleteFriendsByIdFriendYo(
          user_delete,
          self.player.user_id
        );
        self.gameserver.db.deleteFriendsByIdFriendYo(
      ...
      case Types.CLIENT_OPCODE.relationship_change: {
        ...
        let Ava_break = 0;
        self.gameserver.db
          .getUserAvatarsByIdAcc(self.player.user_id)
          .then(function (rowss) {
            ...
            for (var xm in avatar_user) {
              ...
              if (rel_tip === "s") {
                ...
                if (valido) {
                  var name_ava_gift =
                    self.gameserver.avatars.getAvatagift(Ava_break);
                  self.gameserver.pushBroadcastChat(
                    new Message.chatResponse(
                      ...
                      "</3 Has usado el item " +
                        ...
                        ", ahora estás soltero(a) de nuevo, Mejor suerte en tu próxima relación, hay muchos peces en el mar... :(",
                      Types.CHAT_TYPE.SYSTEM
                    ),
                    self.room
                  );
                  self.gameserver.db.deleteAvatarByUserID(
                    self.player.user_id,
                    Ava_break
                  );
                  self.gameserver.db.updateEndRelationByIdAcc(
                    "s",
                    0,
                    self.player.user_id
                  );
                  self.gameserver.db.updateEndRelationByIdAcc(
                    "s",
                    0,
                    self.player.relationship_with_id
                  );
                  //----------------------------------------------------------//
      ...
      case Types.CLIENT_OPCODE.relationship_approved: {
        ...
        if (new_rel_status === "f") {
          //Enamorados
          casar = true;
          self.gameserver.pushBroadcast(
            new Message.chatResponse(
              self,
              "[" +
                self.player.game_id +
                " & " +
                player2.player.game_id +
                "] <3 Nueva Pareja <3 [Server " +
                self.gameserver.id +
                " Room " +
                self.room.id +
                "]",
              Types.CHAT_TYPE.LOVE
            )
          );
        ...
        if (new_rel_status === "m") {
          ...
          if (Date.now() > self.player.gameserverevent) {
            ...
            self.gameserver.db.updateTimeByEventServer(
              Time_Event,
              60,
              "Casamiento",
              self.gameserver.id,
            ...
            self.gameserver.pushBroadcast(
              new Message.chatResponse(
                ...
                "[" +
                  ...
                  " & " +
                  player2.player.game_id +
                  "] <3<3<3 Casados <3<3<3 Para celebrar su matrimonio a 200% GP & GOLD EVENT empezó por 1 hora en el Server " +
                  self.gameserver.id +
                  " - [Server " +
                  self.gameserver.id +
                  " Room " +
            ...
            let Time_Event =
              parseInt(self.player.gameserverevent) + 60 * 1000 * 60;
            self.gameserver.db.updateTimeByEventServer(
              Time_Event,
              60,
              "Casamiento",
            ...
            self.gameserver.pushBroadcast(
              new Message.chatResponse(
                ...
                "[" +
                  ...
                  " & " +
                  player2.player.game_id +
                  "] <3<3<3 Casados <3<3<3 Para celebrar su matrimonio a 200% GP & GOLD EVENT empezó por " +
                  w +
                  " en el Server " +
                  self.gameserver.id +
                  " - [Server " +
        ...
        if (casar) {
          self.gameserver.db.updateRelationStatusByIdAcc(
            new_rel_status,
            from_id,
            self.player.user_id
          );
      ...
      default: {
        ...
        Logger.info(
          "Opcode: " +
            Types.getMessageTypeAsString(opcode) +
            " data: " +
            message
        );
        break;
    ...
    for (var i = 0; i < this.ip_actions[ip].actions.length; i++) {
      if (this.ip_actions[ip].actions[i + 1] != undefined) {
        var time_elapsed =
          this.ip_actions[ip].actions[i + 1] - this.ip_actions[ip].actions[i];
        if (time_elapsed < 500) {
          instant_action++;
        }
      }
  ...
  Chat(msj, ch) {
    ...
    var maxlng = 120;
    if (self.player.gm === 1) maxlng = 150;

    if (msj.length < 1) return null;

    if (msj.length > maxlng) return null;

    if (
      (self.gameserver.name === "Prix" &&
        this.player.gm === 0 &&
        this.player.tournament_start_time_server <= Date.now() &&
        this.player.tournament_end_time_server >= Date.now()) ||
      (self.gameserver.name === "Guilds Prix" &&
        this.player.gm === 0 &&
        this.player.tournament_start_time_server <= Date.now() &&
        this.player.tournament_end_time_server >= Date.now())
      ...
      self.sendMessage(
        new Message.alertResponse(
          "Hola " + this.player.game_id,
          "El Chat en el Lobby esta prohibido para los usuarios."
        )
      );
      return null;
    }
    ...
    if (msj == this.last_chat && this.player.gm === 0) {
      //Codigo De Spam
      self.sendMessage(
        new Message.alert2Response(Types.ALERT2_TYPES.CANT_DUP_CHAT, [])
      );
      return null;
    }
    ...
    if (this.last_message < date) {
      ...
      if (this.strik > 4) {
        ...
        self.gameserver.db.updateMutedByIdAcc(
          FinishDateMuted,
          self.player.user_id
        );
      }
    ...
    if (
      ...
      if (self.player.power_user === 1) type = Types.CHAT_TYPE.POWER_USER;

      if (self.player.gm === 1) type = Types.CHAT_TYPE.GM;

      if (
        (self.player.rank === 28 && self.player.gm === 1) ||
        (self.player.rank === 29 && self.player.gm === 1) ||
        (self.player.rank === 30 && self.player.gm === 1)
      )
      ...
      if (show) {
        if (ch === true) {
          ...
          self.gameserver.pushBroadcastChannel(
            new Message.chatResponse(self, msj, type, this.player.rank)
          );
        } else {
          //console.log('pushBroadcastChat');
          self.gameserver.pushBroadcastChat(
            new Message.chatResponse(self, msj, type),
            self.room
          );
        }
  ...
  saveWinDB() {
    ...
    async function getCurrentRank() {
      try {
        current_rank = await self.gameserver.db.getPlayerCurrentRank(
          self.player.user_id
        );
        previous_rank = await self.gameserver.db.getPlayerPreviousRank(
          self.player.user_id
        );
        processRanks();
      } catch (error) {
    ...
    function awardPrize(gift_rank, cash) {
      self.gameserver.db
        .getBoy(parseInt(self.player.user_id), parseInt(gift_rank))
        .then(function (acc) {})
        .catch(function (err) {
          let datasendgift = {
            UserId: self.player.user_id,
          ...
          self.sendMessage(
            new Message.alert2Response(Types.ALERT2_TYPES.RECEIVED_AVATAR, [
              "FunnyBound",
              gift_rank,
              0,
              "Congratulations you have leveled up and thanks to your effort we will reward you with this avatar",
    ...
    if (self.lucky_egg_left() > 0) lucky_egg_gp = self.player.win_gp;
    ...
    self.player.win_gp = parseInt(
      Math.round(gp_power + plusgp + relation_status + lucky_egg_gp) +
        self.player.win_gp
    );
  ...
  update(tiempo1, tiempo2) {
    ...
    if (self.room) {
      ...
      if (yf === 0) self.player.is_alive = 0;
      else self.player.y = yf;
  ...
  onCreateBot({ positionOfBot, bot_id }) {
    var self = this;
    let id = bot_id;
    let bot_data = {
      user_id: bot_id,
